...
```{r robin - code I need for the plots to work, include = FALSE, cache = TRUE}
clean_data <- read_csv("clean_movie_data.csv") 
...
## re-calculate MAE correctly elastic net
#y_act <- exp(clean_data$gross)
#y_pred <- exp(preds$.pred)
#MAE_orig <- mean(abs(y_act-y_pred))
#MAE_orig

#cv_preds <- collect_predictions(tuned_results)
#cv_preds <- cv_preds |>
 #mutate(y_act = exp(gross)-1,
         #y_pred = exp(.pred)-1)

#MAE_cv <- mean(abs(cv_preds$y_act - cv_preds$y_pred))
#MAE_cv
```
...
The RMSE from cross-validation (38.5 million) and RMSE from prediction (37.7 million) are very 
...
- The baseline/intercept we are given is a gross of ~\$554 million
- The coefficients we see are based on the income compared to the baseline. For example, Tom Hanks would be predicted to earn about \$28.4 million more than the assumed baseline, but Nicole Kidman would be predicted to make about \$27.2 million less than the baseline
- This lasso model identified leading actors was the most influential predictor
- RMSE = \$38.5 million, meaning that on average, this lasso model's predictions for movie revenue is off by about \$38.5 million
...
perf_metrics <- tibble(
  Metric = c("R-Squared", "RMSE (Dollars)", "MAE (Dollars)", "MdAPE (Median Avg % Error)"),
  Value = c(
    r2_binary, 
    ...
    scales::percent(mape_val, accuracy = 1) 
  )
)
...
```{r plots, echo=FALSE, fig.width=7.5, fig.height=4.8, out.width="75%"}
