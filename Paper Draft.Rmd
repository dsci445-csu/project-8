...
ggplot(plot_df, aes(x = Actual, y = Predicted)) +
  ...
  theme_minimal()
```
  The plot shows a clear positive relationship between predicted and actual revenue, but also reveals increasing error for higher-gross films, with the model tending to underestimate blockbuster-level revenues.
...
```
  ...
  The above figure reveals clear heteroskedasticity in the linear regression model. While residuals are centered around zero, indicating no major systematic bias, the spread of residuals is substantially larger for lower fitted log gross values and narrows as fitted values increase. This pattern suggests that the model is less precise when predicting low- and mid-gross movies, but performs more consistently for high-gross films. The lack of strong curvature suggests that a linear relationship is appropriate, but the changing spread of residuals shows that prediction uncertainty varies across revenue levels, motivating the use of more flexible modeling approaches.
...
```{r directorHist, echo=FALSE}
directorHist <- readRDS("TreePlots/DirectorHistogram.rds")
directorHist
```
...
```{r rmsebudget, echo=FALSE}
rmse_budget <- readRDS("TreePlots/rmse_by_budget.rds")
rmse_budget
```
...
```{r}
binned_rf_prevvactual <- readRDS("TreePlots/predicted_vs_actual_plot.rds")
binned_rf_prevvactual 
```
