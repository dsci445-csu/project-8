...
ggplot(movies_fresh_2, aes(x = movie_count)) +
  ...
  labs(
    ...
    subtitle = "Most directors have 1 movie. Look for the 'tail' to set your High/Low cutoff.",
...
```
Quick sanity check of with different encoding FIX VARIBLE NAMES 
```{r}
# 1. Define the Cutoff
cutoff <- 3
...
#    This tells you: "On average, the prediction is off by $X."
...
#    We split movies into 10 buckets (deciles) based on their actual revenue
...
rmse_plot <- ggplot(rmse_by_bin, aes(x = avg_revenue, y = bin_rmse)) +
  ...
  labs(
    title = "Does the Model Struggle with Blockbusters?",
    subtitle = paste0("Global MAE: ", scales::dollar(mae_val), 
                      " | Plot shows RMSE calculated for 10 distinct revenue buckets"),
    x = "Median Revenue of Bin (Log Scale)",
    y = "RMSE of Bin (Log Scale)"
  ) +
  theme_minimal()
